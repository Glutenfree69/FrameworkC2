...
impl<'a> PeParser<'a> {
    ...
    pub fn parse(data: &'a [u8]) -> Result<Self, ParseError> {
        ...
        let dos_header: DosHeader = unsafe {
            ptr::read_unaligned(data.as_ptr() as *const DosHeader)
        };
        ...
        // Lire les NT Headers
        let nt_headers: NtHeaders64 = unsafe {
            ptr::read_unaligned(data.as_ptr().add(pe_offset) as *const NtHeaders64)
        };
        ...
        for i in 0..num_sections {
            let section_ptr = data.as_ptr() as usize 
                + sections_offset 
                + i * std::mem::size_of::<SectionHeader>();
            
            if section_ptr + std::mem::size_of::<SectionHeader>() > data.as_ptr() as usize + data.len() {
                return Err(ParseError::InvalidSection(format!("Section {} out of bounds", i)));
            }
            
            let section: SectionHeader = unsafe {
                ptr::read_unaligned(section_ptr as *const SectionHeader)
            };
    ...
    pub fn read_struct_at_rva<T: Copy>(&self, rva: u32) -> Option<T> {
        ...
        if offset + std::mem::size_of::<T>() <= self.raw.len() {
            Some(unsafe {
                ptr::read_unaligned(self.raw.as_ptr().add(offset) as *const T)
            })
        } else {
            None
        }
