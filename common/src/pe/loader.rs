...
use super::parser::{ParseError, PeParser};
use super::structs::*;
...
impl PeLoader {
    ...
    unsafe fn allocate_memory(size: usize, preferred_base: u64) -> Result<*mut u8, LoadError> {
        ...
        let addr = Self::virtual_alloc(ptr::null_mut(), size, MEM_COMMIT_RESERVE, PAGE_READWRITE);
    ...
    unsafe fn protect_sections(pe: &PeParser, base: *mut u8) -> Result<(), LoadError> {
        for section in &pe.sections {
            ...
            let result =
                Self::virtual_protect(addr as *mut CVoid, size, protection, &mut old_protect);
    ...
    unsafe fn call_tls_callbacks(
        ...
        let tls: TlsDirectory64 = pe
            .read_struct_at_rva(tls_dir.virtual_address)
    ...
    unsafe fn call_entry_point(&mut self, reason: u32) -> Result<(), LoadError> {
        if let Some(entry_point) = self.entry_point {
            let result = entry_point(self.base_address as *mut CVoid, reason, ptr::null_mut());
    ...
    unsafe fn register_exception_table(pe: &PeParser, base: *mut u8) -> Result<u32, LoadError> {
        ...
        let function_table =
            base.add(exception_dir.virtual_address as usize) as *mut RUNTIME_FUNCTION;

        // Enregistrer la table via RtlAddFunctionTable
        let result = Self::rtl_add_function_table(function_table, entry_count, base as u64);
    ...
    unsafe fn rtl_add_function_table(
        ...
        let proc = GetProcAddress(ntdll, c"RtlAddFunctionTable".as_ptr());
        if proc.is_null() {
            return false;
        }
    ...
    unsafe fn load_library(name: &str) -> Result<*mut CVoid, LoadError> {
        ...
        let c_name = CString::new(name).map_err(|_| LoadError::ModuleNotFound(name.to_string()))?;
    ...
    unsafe fn get_proc_address(module: *mut CVoid, name: &str) -> Result<*mut CVoid, LoadError> {
        ...
        let c_name = CString::new(name).map_err(|_| LoadError::FunctionNotFound {
